...
Do not use markdown formatting like \`\`\`json. Return only valid JSON.`
  ...
  async generateRenderImage(
    ...
    try {
      ...
      // If we have a base image, use it as context (image-to-image/reference)
      ...
      if (!response.candidates || response.candidates.length === 0) {
        const errorDetail = (response as any).promptFeedback?.blockReason 
          ? `Safety Blocked: ${(response as any).promptFeedback.blockReason}`
